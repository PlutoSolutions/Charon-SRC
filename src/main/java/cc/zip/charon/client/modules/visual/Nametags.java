...
import me.dev.mexen.api.mixin.mixins.render.IRenderManager;
...
@ModuleManifest(label="Nametags", category= Module.Category.VISUAL, listen=false)
...
extends Module {
    ...
    private final Setting<Boolean> rectBorder = this.register(new Setting<Boolean>("Border", Boolean.valueOf(true), v -> this.rect.getValue()));
    private final Setting<Boolean> gradientBorder = this.register(new Setting<Boolean>("GradientBorder", Boolean.valueOf(false), v -> this.rectBorder.getValue()));
    ...
    public void onRender3D() {
        ...
        camera.setPosition(Objects.requireNonNull(this.mc.getRenderViewEntity()).posX, this.mc.getRenderViewEntity().posY, this.mc.getRenderViewEntity().posZ);
        int size = this.mc.world.playerEntities.size();
        for (int i = 0; i < size; ++i) {
            EntityPlayer player = (EntityPlayer)this.mc.world.playerEntities.get(i);
            if (!camera.isBoundingBoxInFrustum(player.getEntityBoundingBox()) || player == this.mc.player || player.isDead || !(player.getHealth() > 0.0f)) continue;
            double x = this.interpolate(player.lastTickPosX, player.posX, this.mc.getRenderPartialTicks()) - ((IRenderManager)this.mc.getRenderManager()).getRenderPosX();
            double y = this.interpolate(player.lastTickPosY, player.posY, this.mc.getRenderPartialTicks()) - ((IRenderManager)this.mc.getRenderManager()).getRenderPosY();
            double z = this.interpolate(player.lastTickPosZ, player.posZ, this.mc.getRenderPartialTicks()) - ((IRenderManager)this.mc.getRenderManager()).getRenderPosZ();
            this.renderNameTag(player, x, y, z, this.mc.getRenderPartialTicks());
        }
    }
    ...
    private void renderNameTag(EntityPlayer player, double x, double y, double z, float delta) {
        ...
        int width = this.mc.fontRenderer.getStringWidth(displayTag) / 2;
        ...
        if (this.rect.getValue().booleanValue()) {
            if (this.gradientBorder.getValue().booleanValue()) {
                ...
                RenderUtil.drawBorderedRect(-width - 2, -(this.mc.fontRenderer.FONT_HEIGHT + 1), (float)width + 2.0f, 1.5f, 0x55000000, this.rectBorder.getValue().booleanValue() ? (Charon.INSTANCE.getFriendManager().isFriend(player) ? -11157267 : Charon.INSTANCE.getColorManager().getColorAsInt()) : 0);
            }
        }
        ...
        if (this.armor.getValue().booleanValue()) {
            GlStateManager.pushMatrix();
            int xOffset = -8;
            int size = player.inventory.armorInventory.size();
            for (int i = 0; i < size; ++i) {
                xOffset -= 8;
            }
        ...
        this.mc.fontRenderer.drawStringWithShadow(displayTag, (float)(-width), (float)(-(this.mc.fontRenderer.FONT_HEIGHT - 1)), this.getDisplayColour(player));
    ...
    private void renderEnchantmentText(ItemStack stack, int x, int y) {
        ...
        for (int index = 0; index < enchants.tagCount(); ++index) {
            ...
            this.mc.fontRenderer.drawStringWithShadow(encName, (float)(x * 2), (float)enchantmentY, -1);
            enchantmentY -= 8;
        }
        if (Nametags.hasDurability(stack)) {
            ...
            this.mc.fontRenderer.drawStringWithShadow(color + percent + "%", (float)(x * 2), (float)enchantmentY, -1);
        }
    }
    ...
    private String getDisplayTag(EntityPlayer player) {
        ...
        if (this.totemPops.getValue().booleanValue()) {
            Map<String, Integer> registry = Charon.INSTANCE.getPopManager().getPopMap();
            popStr = popStr + (registry.containsKey(player.getName()) ? " -" + Charon.INSTANCE.getPopManager().getPopMap().get(player.getName()) : "");
        }
        name = name + color + " " + (int)health;
        return name + " " + (Object)ChatFormatting.RESET + pingStr + popStr;
    }

    private int getDisplayColour(EntityPlayer player) {
        if (Charon.INSTANCE.getFriendManager().isFriend(player)) {
            return -11157267;
        }
