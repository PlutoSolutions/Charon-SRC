...
import cc.zip.charon.api.property.EnumConverter;
...
public class ConfigManager {
    ...
    public String config = "Charon/config/";

    public void loadConfig(String name) {
        List files = Arrays.stream(Objects.requireNonNull(new File("Charon").listFiles())).filter(File::isDirectory).collect(Collectors.toList());this.config = files.contains(new File("Charon/" + name + "/")) ? "Charon/" + name + "/" : "Charon/config/";
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("Charon/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("Charon", ""));
                writer.close();
            } else {
    ...
    public void init() {
        this.features.addAll(Charon.INSTANCE.getModuleManager().getModules());
